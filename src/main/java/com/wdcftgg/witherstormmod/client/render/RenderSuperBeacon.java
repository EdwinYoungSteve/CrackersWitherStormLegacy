...
public class RenderSuperBeacon extends TileEntitySpecialRenderer<TileEntityAbstractSuperBeacon> {
    ...
    private static final Map<TileEntitySuperBeacon.SupportColor, ResourceLocation> SUPPORT_TEXTURES =
            new EnumMap<TileEntitySuperBeacon.SupportColor, ResourceLocation>(
                    TileEntitySuperBeacon.SupportColor.class);

    static {
        SUPPORT_TEXTURES.put(TileEntitySuperBeacon.SupportColor.AQUA,
                new ResourceLocation(Tags.MOD_ID, "textures/block/support_beacon_diamond.png"));
        SUPPORT_TEXTURES.put(TileEntitySuperBeacon.SupportColor.GREEN,
                new ResourceLocation(Tags.MOD_ID, "textures/block/support_beacon_emerald.png"));
        SUPPORT_TEXTURES.put(TileEntitySuperBeacon.SupportColor.GRAY,
                new ResourceLocation(Tags.MOD_ID, "textures/block/support_beacon_iron.png"));
        SUPPORT_TEXTURES.put(TileEntitySuperBeacon.SupportColor.RED,
    ...
    @Override
    public void render(TileEntityAbstractSuperBeacon beacon, double x, double y, double z,
        ...
        if (beacon instanceof TileEntitySuperSupportBeacon) {
            TileEntitySuperSupportBeacon support = (TileEntitySuperSupportBeacon) beacon;
            if (animation > 0.001F) renderSupportLink(support, x, y, z, animation);
            if (support.showWorkingArea()) renderSupportArea(support, x, y, z);
        } else {
            TileEntitySuperBeacon main = (TileEntitySuperBeacon) beacon;
            ...
            if (main.isResummoningWitherStorm()
                    && main.getResummonTicks() > LegacySuperBeaconLogic.RESUMMON_START) {
    ...
    private void renderCrystal(TileEntityAbstractSuperBeacon beacon, double x, double y, double z,
        ...
        if (beacon instanceof TileEntitySuperSupportBeacon) {
            ...
            TileEntitySuperBeacon.SupportColor color =
                    ((TileEntitySuperSupportBeacon) beacon).getColor();
            bindTexture(color == null ? SUPPORT_TEXTURES.get(TileEntitySuperBeacon.SupportColor.GRAY)
    ...
    private static float crystalScale(TileEntityAbstractSuperBeacon beacon, float partialTicks) {
        float animation = beacon.getActivationAnimation(partialTicks);
    ...
    private static void renderOrb(float scale, float minU, float minV, float maxU, float maxV,
                                  TileEntityAbstractSuperBeacon beacon) {
    ...
    private void renderMainItems(TileEntitySuperBeacon beacon, double x, double y, double z,
                                 float partialTicks) {
        ...
        float scale = beacon.getResummonTicks() > 0
                ? LegacySuperBeaconLogic.getResummonItemScale(resummonTick) : 1.0F;
    ...
    private void renderResummonCommandBlock(TileEntitySuperBeacon beacon, double x, double y, double z,
                                             float partialTicks) {
        float tick = beacon.getTicks() + partialTicks;
    ...
    private void renderVerticalBeam(TileEntityAbstractSuperBeacon beacon, double x, double y, double z,
                                    float partialTicks, float animation) {
    ...
    private void renderSupportLink(TileEntitySuperSupportBeacon support, double x, double y, double z,
                                   float animation) {
    ...
    private void renderSupportArea(TileEntitySuperSupportBeacon support, double x, double y, double z) {
        ...
        float center = LegacySuperBeaconLogic.angleDegrees(
    ...
    @Override
    public boolean isGlobalRenderer(TileEntityAbstractSuperBeacon beacon) {
        return true;
    }
}
