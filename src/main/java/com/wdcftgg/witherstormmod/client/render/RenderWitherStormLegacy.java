...
import com.wdcftgg.witherstormmod.client.model.LegacyWitherStormPhaseModel;
import com.wdcftgg.witherstormmod.client.model.LegacyWitherStormPhaseModel.Form;
...
public class RenderWitherStormLegacy extends RenderLiving<EntityWitherStormLegacy> {
    ...
    private final Map<Form, LegacyWitherStormPhaseModel> models = new EnumMap<Form, LegacyWitherStormPhaseModel>(Form.class);

    public RenderWitherStormLegacy(RenderManager renderManager) {
        super(renderManager, new LegacyWitherStormPhaseModel(Form.COMMAND_BLOCK), 3.0F);
        models.put(Form.COMMAND_BLOCK, (LegacyWitherStormPhaseModel) mainModel);
        for (Form form : Form.values()) {
            if (form != Form.COMMAND_BLOCK) models.put(form, new LegacyWitherStormPhaseModel(form));
        }
    }

    @Override
    public void doRender(EntityWitherStormLegacy entity, double x, double y, double z, float entityYaw, float partialTicks) {
        mainModel = models.get(fetchForm(entity));
        super.doRender(entity, x, y, z, entityYaw, partialTicks);
    }

    @Override
    protected ResourceLocation getEntityTexture(EntityWitherStormLegacy entity) {
        return TEXTURE;
    }

    @Override
    protected void preRenderCallback(EntityWitherStormLegacy entity, float partialTickTime) {
        float scale = 2.0F;
    ...
    @Override
    protected void applyRotations(EntityWitherStormLegacy entity, float ageInTicks,
                                  float rotationYaw, float partialTicks) {
        GlStateManager.rotate(180.0F - rotationYaw, 0.0F, 1.0F, 0.0F);
        GlStateManager.rotate(entity.getBodyXRotation(partialTicks), 1.0F, 0.0F, 0.0F);
    }

    private static Form fetchForm(EntityWitherStormLegacy entity) {
