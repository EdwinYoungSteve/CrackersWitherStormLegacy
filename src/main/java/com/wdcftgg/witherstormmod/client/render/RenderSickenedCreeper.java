...
public class RenderSickenedCreeper extends RenderSickenedMob<SickenedEntities.SickenedCreeper> {
    public RenderSickenedCreeper(RenderManager manager) {
        super(manager, new ModelCreeper(), 0.5F, "textures/entity/sickened/sickened_creeper.png");
        addLayer(new LayerSickenedCreeperCharge(this));
    }

    @Override
    protected void preRenderCallback(SickenedEntities.SickenedCreeper entity, float partialTicks) {
    ...
    @Override
    protected int getColorMultiplier(SickenedEntities.SickenedCreeper entity, float brightness, float partialTicks) {
        float swelling = entity.getCreeperFlashIntensity(partialTicks);
