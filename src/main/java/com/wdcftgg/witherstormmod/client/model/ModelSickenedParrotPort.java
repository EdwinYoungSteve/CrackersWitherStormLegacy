...
public class ModelSickenedParrotPort extends ModelBase
{
    ModelRenderer body;
    ModelRenderer tail;
    ...
    private ModelSickenedParrotPort.State state = ModelSickenedParrotPort.State.STANDING;

    public ModelSickenedParrotPort()
    {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn)
    {
        ...
        if (this.state != ModelSickenedParrotPort.State.FLYING)
        {
            if (this.state == ModelSickenedParrotPort.State.SITTING)
            {
                return;
            }

            if (this.state == ModelSickenedParrotPort.State.PARTY)
            {
    ...
    public void setLivingAnimations(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTickTime)
    {
        ...
        if (entitylivingbaseIn instanceof SickenedEntities.SickenedParrot)
        {
            SickenedEntities.SickenedParrot entityparrot = (SickenedEntities.SickenedParrot) entitylivingbaseIn;

            if (entityparrot.isSickenedPartying())
            {
                this.legLeft.rotateAngleZ = -0.34906584F;
                this.legRight.rotateAngleZ = 0.34906584F;
                this.state = ModelSickenedParrotPort.State.PARTY;
                return;
            }
            ...
            if (entityparrot.isSickenedSitting())
            {
                ...
                this.state = ModelSickenedParrotPort.State.SITTING;
            }
            else if (entityparrot.isSickenedFlying())
            {
                ...
                this.state = ModelSickenedParrotPort.State.FLYING;
            }
            else
            {
                this.state = ModelSickenedParrotPort.State.STANDING;
            }
