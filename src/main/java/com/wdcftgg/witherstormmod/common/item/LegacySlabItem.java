...
public class LegacySlabItem extends ItemBlock {

    private final LegacySlabBlock slab;

    public LegacySlabItem(LegacySlabBlock slab) {
        super(slab);
        this.slab = slab;
    }
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand,
        ...
        if (state.getBlock() == slab) {
            LegacySlabBlock.Half half = state.getValue(LegacySlabBlock.HALF);
            boolean clickedJoinFace = half == LegacySlabBlock.Half.BOTTOM && facing == EnumFacing.UP
                    || half == LegacySlabBlock.Half.TOP && facing == EnumFacing.DOWN;
    ...
    public boolean canPlaceBlockOnSide(World world, BlockPos pos, EnumFacing side, EntityPlayer player, ItemStack stack) {
        IBlockState state = world.getBlockState(pos);
        if (state.getBlock() == slab && state.getValue(LegacySlabBlock.HALF) != LegacySlabBlock.Half.DOUBLE) {
            LegacySlabBlock.Half half = state.getValue(LegacySlabBlock.HALF);
            if (half == LegacySlabBlock.Half.BOTTOM && side == EnumFacing.UP
                    || half == LegacySlabBlock.Half.TOP && side == EnumFacing.DOWN) return true;
        }
        IBlockState adjacent = world.getBlockState(pos.offset(side));
        return adjacent.getBlock() == slab && adjacent.getValue(LegacySlabBlock.HALF) != LegacySlabBlock.Half.DOUBLE
    ...
    private boolean merge(EntityPlayer player, World world, BlockPos pos, ItemStack stack) {
        ...
        IBlockState merged = slab.getDefaultState().withProperty(LegacySlabBlock.HALF, LegacySlabBlock.Half.DOUBLE);
